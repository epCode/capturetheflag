...
local max = {hp = 20}
local players = {}
-- [pname] = true for players whose over-head HP bar should be hidden (e.g. spectators)
local hidden = {}
...
-- Hide or show a player's over-head HP bar. has_entity() consults can_show() and
-- removes the entity when hidden, or recreates it when shown again.
function hpbar.set_hidden(player, is_hidden)
	player = PlayerObj(player)
	hidden[player:get_player_name()] = is_hidden or nil
...
function hpbar.can_show(player)
	return not hidden[PlayerName(player)]
end
